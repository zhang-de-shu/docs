...
### 1.1.2 怎么保障标注一致性?(从"靠自觉"到"可度量、可闭环")

上面说"一致性极关键",但**一致性不是靠叮嘱出来的,而是靠机制 + 度量 + 闭环管出来的**。核心思路一句话:

> **把"不一致"当成"指南有歧义"的信号,而不是"标注员不行"的信号。** 业界共识:低一致性几乎总是指南模糊/任务本身主观/边界 case 没写清,而非标注员粗心——所以处置动作应是**改指南**,而不是加更多 QA 抽查。

注意一个 SFT 特有的区分:SFT 数据里**自由生成的 response(text production)**不适合用一致性系数去算(没有"标准答案"可对齐),而**评分/分级/选择类**的标注(rubric 打分、A/B 偏好、分级标签)才适用 IAA 度量。所以一致性保障要**分两条线**走。

**第一条线:可度量的标注(打分/分级/偏好)——用 IAA 量化 + 阈值门控**

- **量化指标怎么选**:
  - **Cohen's κ**:两个标注员、离散类别;主流门槛 **κ > 0.7**(更严的生产线要 **> 0.8**)。
...
- **写带锚点的 rubric(最关键的工具)**:把抽象目标翻译成可操作判据。例:"要事实准确" → "**逐句 fact-check,有错误事实则该回答记 2/5**"。Likert 量表(5 或 7 级)**必须给每一档明确锚点**,否则会出现"居中倾向"和各人理解漂移。
  - **一份 rubric,两处复用**:标注阶段用它统一人;模型训练完后,它直接变成 **LLM-as-judge 的评分蓝图**——数据生产和评估闭环对齐(详见评估手册)。
- **给正反范例**:SFT 项目要同时给**高质量与低质量**示例,明确"什么算好的 response、什么算坏的",而不只给一个理想答案。
- **统一风格的具体抓手**:同类问题的**结构模板(段落顺序/标题/列表)、语气、格式约束(是否 markdown、代码块规范)、拒答措辞**都写进 style guide,并配可自动化的校验(格式 lint、JSON schema、代码 lint)。

**贯穿两条线的工作流(端到端可照做):**

1. **先写指南 + rubric + 边界 case 范例**,别一上来就量产。
2. **校准轮(calibration round)**:全体标注员先在 **5–10% 或 ≥100–200 条**的种子集上各自标,然后开会对齐分歧 case。
3. **跑 IAA**:序数用加权 α、类别用 κ/α;**达 >0.8 且错误率 <10% 才放量**;否则改指南再跑一个小批 pilot。
4. **分层评审**:标注 → 同行复审 → 专家裁决(adjudication)。数据显示:**单遍标注在主观任务上错误率 15–25%;加分层评审能压到 <5%**,代价是标注时间多 30–40%——对要进生产的训练数据值得。
5. **裁决产出最终标签**:常用"**2 标注员 + 1 资深裁决**";分歧的洞察**回写进指南**(把指南当**活文档**,边界 case 不断补)。
6. **定期再校准**抗漂移:有实践报告"**让讨论自动更新指南并触发重训,一致率从 72% 提升到 91%**"。

> **诊断分歧的模式**(把分歧当工具用):**孤立的分歧**多半是注意力失误(培训/抽查);**整类别的分歧**是指南不清(改指南);**专家之间持续分歧**是任务本身就主观(要重新界定任务或接受一定噪声)。
> ⚠️ 一个常见反面教训:很多团队**算了 IAA 却不解释它**,或样本量太小得不出统计结论。一致性度量要**配合混淆矩阵/错误分类法**去定位根因,而不是只报一个数字。
