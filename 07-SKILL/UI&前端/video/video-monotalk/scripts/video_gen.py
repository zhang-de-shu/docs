...
VIDEO_PROMPT = "台词仅仅用于声音，不要出现在屏幕上。讲解员正面面对镜头，自然温和微笑，随内容做出自然且轻微的动作。"
...
def create_video_task(prompt, first_frame_base64, segment_index,
    ...
    payload = {
        ...
        "watermark": False,
        "generate_audio": True,
        "negative_prompt": "text, letters, words, numbers, symbols, subtitles, titles, captions, watermark, logo, UI, icons, overlay, particle effects, glow, lens flare, charts, code"
    }
...
def generate_video_segment(prompt, segment_index, dialogue, start_frame_path):
    ...
    # 构建 prompt
    no_overlay_desc = "视频画面中绝对不允许出现任何文字、字母、数字、符号、字幕、标题、Logo、水印、UI元素、特效、光效。"
    framing_desc = "首帧图片和参考图必须完全一致。全程固定机位，严禁任何镜头运动（推拉摇移、缩放、环绕等），画面构图从头到尾保持不变。"
    end_pose_desc = "结束姿态：视频最后1-2秒，讲解员回到与首帧一致的正面朝向，保持微笑。"
    lighting_desc = "人物肤色全程保持冷白皮，人物形象外貌、背景颜色和参考图需要完全一致，画面中只允许出现人物和纯净的单色背景。"
...
SUBTITLE_MAX_CHARS = 18  # 每行字幕最大显示字数，超过则换行
...
def render_title_frame(title_text, width, height):
    ...
    draw = ImageDraw.Draw(img)
    main_font = ImageFont.truetype(font_path, TITLE_FONT_SIZE)
...
def main():
    ...
    for item in raw_segments:
        ...
        if page == 0:
            ...
            segments.append({"segment_index": page - 1, "dialogue": content})
    ...
    for i, segment in enumerate(segments):
        ...
        tasks.append({"index": i, "dialogue": dialogue, "prompt": full_prompt})
