 ...
 * 配置（优先级：命令行 > 环境变量 > 默认）：
 *   --key       ZENMUX_API_KEY
 *   --model     ZENMUX_IMAGE_MODEL（默认 meta/muse-image-1.0）
 *   --chat-model ZENMUX_CHAT_MODEL（默认 qwen/qwen3.7-flash，仅 --qc 用）
 ...
 *   openai/* meta/* → /images/generations（无参考图）或 /images/edits（有参考图，多图用 image[] 重复传）
 *   其他            → vertex-ai :predict（参考图仅取首张）
 *
 * 环境变量自动加载：脚本目录（或上溯）若有 .env 会自动读取
 */
import fs from 'fs'
...
async function main() {
  ...
  } else if (MODEL.startsWith('openai/') || MODEL.startsWith('meta/')) {
    if (refs.length) {
  ...
  console.log(`[gen_image] 已保存: ${OUT} (${sniffImageMime(b64) || mime}, ${Math.round(b64.length * 3 / 4 / 1024)}KB, refs=${refs.length})`)
