...
- **玩家就是主角，不是读者在读小说**：
  - 旁白（script 中 kind=narration 的节拍）→ **第二人称「你」**写主角；禁止用「他/沈砚」指代主角。
  - 独白（script 中 kind=monologue 的节拍）→ **第一人称「我」**（现在时口语，≤30 字）。
  - 对白（script 中 kind=dialogue 的节拍）→ **不改人称**；引号 `「…」` 内按说话人原话保留。
  - `script` 是按演出顺序排列的节拍序列，旁白/对白/独白可任意穿插，禁止写成"先旁白后对白"的割裂结构。
