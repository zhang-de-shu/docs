...
用户的输入是种子——可能是一句话（"武侠世界，盟主离奇死亡"）、一个场景（"雨夜，一个人敲开灵堂的门"）、或一个开头（"我醒来时手里握着凶器"）。AI 从种子直接生成完整的故事线框架。规模遵守 `domain-baseline.md` §2.1 的基准。
...
6. **种子不清时**：给用户 2-3 个方向选项（不同类型/基调/真相走向），让用户挑选后再细化。
