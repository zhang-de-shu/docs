...
3. **五位专家导演终审（validate:director_review，可选）**：用户需要更严格创作评审时触发。五位评委各打 0-10 分，observation 必须引用具体节点标题，产出总分、greenlit 判定、mustFix 清单、standout_moment 高光点评。verdicts 数组必须恰好 5 项：
    ...
    - 麦基（结构完整性）：三幕结构是否成立？中点大反转是否存在？最终选择是否是全片最难的道德抉择？依据：故事脊柱强度、张力节奏、选择的戏剧代价。
    - 乔布斯（产品体验）：玩家是否感到自己在做真实有意义的选择？有没有"one more thing"时刻——让一切重新被理解的反转？依据：选择的感知权重、体验流畅性、惊喜感。
    ...
    - 输出模版：{"verdicts":[{"lens":"斯皮尔伯格·情感冲击力","score":7,"observation":"必须引用具体节点标题，指出哪里扎实、哪里不足","note":"一条可执行的改进建议"}，…共5项],"overallScore":7,"greenlit":false,"executiveSummary":"项目…核心问题是…修复后建议绿灯。","mustFix":["逐条列出必须修改项，引用具体章节幕/节点"],"standout_moment":"全剧目前最精彩的单一时刻——引用具体台词，说明它完成了什么"}
