...
3. **直接从用户种子生成**：不做世界观/规则/角色问卷。用户的输入（一句话、一个场景、或一个开头）就是全部素材，AI 从中提炼并生成完整故事线框架；唯一遵守的硬约束是 `references/domain-baseline.md` §2.1 的规模基准。
4. **唯一硬性检查是脚本闭环校验**：分支无死路、无困死、结局可达（error=0）。写作质量原则（真实感、对白、独白）只是指导，不是关卡。

# 执行步骤

## 1. 接收用户种子
用户可能给一句话、一个场景、或一个开头——都直接进入阶段一。种子信息不足时，可以给用户 2-3 个方向选项（不同类型/基调/真相走向）让用户挑选，除此之外不向用户提问。
...
| 阶段 | 子技能目录 | 产出 |
|------|-----------|------|
| 一、故事线框架 | `references/01-story-framework.md` | 从种子直接生成：故事核心 + 每章幕结构（每章几幕、各幕内容）+ 结局设计（对应章节映射）+ 精简变量 + 主角 fatalFlaw + 回响映射 |
